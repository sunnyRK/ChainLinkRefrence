...
class LiquidityContainer extends Component {
  ...
  //* * Calculate Second token quantity and price by chainlink oracles **//
  async CalculateSecondTokenQuantityByChainlinkOracle() {
    // Fethces Price of Token pair from TELLOR ORACLES
    const oracleInstance = await getChainlinkOracle(web3);

    const price1 = await oracleInstance.methods.getLatestPrice(this.state.liquidityToken0).call();
    const price2 = await oracleInstance.methods.getLatestPrice(this.state.liquidityToken1).call();
  ...
  handleInputPair = async (e, { value }) => {
    if (this.state.liquidityToken0 != '') {
      ...
      // Calculate Second token quantity and price by chainlink oracles
      await this.CalculateSecondTokenQuantityByChainlinkOracle();
    } else {
