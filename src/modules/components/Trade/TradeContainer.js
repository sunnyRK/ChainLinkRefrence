...
class TradeContainer extends Component {
  ...
  async calculateSLippageRateFromChainlinkOracle() {
    // Fethces Price of Token pair from TELLOR ORACLES
    const oracleInstance = await getChainlinkOracle(web3);
    const price1 = await oracleInstance.methods.getLatestPrice(this.state.token0).call();
    const price2 = await oracleInstance.methods.getLatestPrice(this.state.token1).call();

    console.log('Chainlink price1: ', price1);
    console.log('Chainlink price2: ', price2);

    // const price1 = "1002975"; //DAI
    // const price2 = "206000"; //BAT
    // const price2 = "336600"; //ZRX

    const token0Price = parseInt(price1) / 100000000;
    const token1Price = parseInt(price2) / 100000000;
    ...
    this.setState({
      slippage: slippage2,
      chainlinkRate: token1Quantity / 1e18,
      chainlinkRateWithToken: token1Quantity/1e18 + " " + this.state.token1
    });
  }
  ...
  async getAmountOutValue() {
    ...
    const reserves = await pairInstance.methods.getReserves().call();
      ...
      await this.calculateSLippageRateFromChainlinkOracle();
  }
      ...
      if(parseInt(reserves[0]) >= this.state.swapValinWei) {
        if (this.state.token0 != '') {
          ...
          if (this.state.swapValinWei > 0) {
            ...
            console.log(this.state.chainlinkRate);
            if (parseFloat(this.state.amountOut) >= parseFloat(this.state.chainlinkRate)) {
              // trade directly
              console.log('All set to go');
              this.setState({ shouldSwap: true });
            } else {
              if (parseInt(this.state.slippage) <= 2) {
                this.setState({ shouldSwap: true });
              } else {
  ...
  render() {
    const {
      ...
      chainlinkRate, amountInBalanceText, token0, reserve0, blocking, chainlinkRateWithToken, amountOutWithToken
    } = this.state;
    return (
      <Trade
        ...
        chainlinkRateWithToken={chainlinkRateWithToken}
        slippage={slippage}
